...
func (config *Config) CreateConnectionClient() (*mongo.Client, error) {
	...
	if config.Username == "" || config.Password == "" {
		client, err = mongo.NewClient(options.Client().ApplyURI(config.Address))
	} else {
		client, err = mongo.NewClient(options.Client().SetAuth(
			options.Credential{
				Username:   config.Username,
				Password:   config.Password,
				AuthSource: config.AuthSource}).ApplyURI(config.Address))
	}
...
func (ci *MongoInstance) GetConfig() *Config {
	return ci.conf
}

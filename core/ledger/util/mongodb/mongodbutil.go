...
func CreateMongoInstance(config *Config, metricsProvider metrics.Provider) (*MongoInstance, error) {
	...
	if config.Username == "" || config.Password == "" {
		client, err = mongo.NewClient(options.Client().ApplyURI(finalURL.String()))
	} else {
		client, err = mongo.NewClient(options.Client().SetAuth(
			options.Credential{Username: config.Username, Password: config.Password, AuthSource: config.AuthSource}).ApplyURI(finalURL.String()))
	}
	mongoInstance := &MongoInstance{conf: config, client: client}

...
func TestHandleChaincodeDeploy(t *testing.T) {
	...
	dbArtifactsTarBytes := testutil.CreateTarBytesForTest(
		[]*testutil.TarFileEntry{
			{Name: "META-INF/statedb/mongodb/indexes/indexColorSortName.json", Body: `{"index":{"fields":[{"color":-1}]}, "name":"indexColorSortName"}`},
			{Name: "META-INF/statedb/mongodb/indexes/indexSizeSortName.json", Body: `{"index":{"fields":[{"size":-1}]}, "name":"indexSizeSortName"}`},
			{Name: "META-INF/statedb/mongodb/collections/collectionMarbles/indexes/indexCollMarbles.json", Body: `{"index":{"fields":["docType","owner"]}, "name":"indexCollectionMarbles"}`},
			{Name: "META-INF/statedb/mongodb/collections/collectionMarblesPrivateDetails/indexes/indexCollPrivDetails.json", Body: `{"index":{"fields":["docType","price"]}, "name":"indexPrivateDetails"}`},
		},
	)

	//Create a query
	queryString := `{"query":{"owner":"fred"}}`
	//
	_, err = db.ExecuteQuery("ns1", queryString)
	assert.NoError(t, err)

	//Create a query with a sort
	queryString = `{"query":{"owner":"fred"}, "sort": {"size": -1}}`
	...
	fileEntries, errExtract := ccprovider.ExtractFileEntries(dbArtifactsTarBytes, "mongodb")
	assert.NoError(t, errExtract)

	indexCapable.ProcessIndexesForChaincodeDeploy("ns1", fileEntries[filepath.FromSlash("META-INF/statedb/mongodb/indexes")])
...
func TestHandleChaincodeDeployErroneousIndexFile(t *testing.T) {
	...
	var dbArtifactsTarBytes []byte

	dbArtifactsTarBytes = testutil.CreateTarBytesForTest(
		[]*testutil.TarFileEntry{
			{Name: "META-INF/statedb/mongodb/indexes/indexSizeSortName.json", Body: `{"key":{"size":-1}, "name":"indexSizeSortName"}`},
			{Name: "META-INF/statedb/mongodb/indexes/badSyntax.json", Body: badSyntaxFileContent},
		},
	)
	...
	time.Sleep(100 * time.Millisecond)
	//Query should complete without error
	_, err = db.ExecuteQuery("ns1", `{"query":{"owner":"fred"}, "sort": {"size": -1}}`)
	assert.NoError(t, err)
}
...
func TestPaginatedQuery(t *testing.T) {
	...
	// Create a query with a sort
	queryString = `{"query":{"color":"red"}, "sort":{"size": 1}}`
	...
	skip, err := executeQuery(t, db, "ns1", queryString, "", int32(10), returnKeys)
	assert.NoError(t, err)
	returnKeys = []string{"key17", "key18", "key19", "key20", "key22", "key24", "key25", "key26", "key28", "key29"}
	skip, err = executeQuery(t, db, "ns1", queryString, skip, int32(10), returnKeys)
	assert.NoError(t, err)

	returnKeys = []string{"key30", "key32", "key33", "key34", "key35", "key37", "key39", "key40"}
	_, err = executeQuery(t, db, "ns1", queryString, skip, int32(10), returnKeys)
	assert.NoError(t, err)
	...
	viper.Set("ledger.state.mongoDBConfig.queryLimit", 50)
	...
	viper.Set("ledger.state.mongoDBConfig.queryLimit", 10)
	...
	//Set queryLimit to 5
	viper.Set("ledger.state.mongoDBConfig.queryLimit", 5)
	...
	viper.Set("ledger.state.mongoDBConfig.queryLimit", 1000)
}

func executeQuery(t *testing.T, db statedb.VersionedDB, namespace, query, skip string, limit int32, returnKeys []string) (string, error) {

	var itr statedb.ResultsIterator
	var err error

	if limit == int32(0) && skip == "" {
	...
	if queryResultItr, ok := itr.(statedb.QueryResultsIterator); ok {
		returnSkip = queryResultItr.GetBookmarkAndClose()
	}

	return returnSkip, nil
}
...
func TestRangeScanWithMongoInternalDocsPresent(t *testing.T) {
	...
	env.init(t)
	defer env.cleanup()
	db, err := env.DBProvider.GetDBHandle("testrangescanfiltermongointernaldocs")
	assert.NoError(t, err)
	mongoDatabase, err := db.(*VersionedDB).getNamespaceDBHandle("ns")
	assert.NoError(t, err)
	db.Open()
	defer db.Close()
	_, err = mongoDatabase.CreateIndex(`{
	...
	s, err = newQueryScanner("ns", mongoDatabase, "", 2, 2, "", "_", "")
	assert.NoError(t, err)
	assertQueryResults(t, s.resultsInfo.results, []string{"key-1", "key-2"})
	assert.Equal(t, "key-3", s.queryDefinition.startKey)
}

func assertQueryResults(t *testing.T, results []*mongodb.QueryResult, expectedIds []string) {

...
func NewVersionedDBProvider(config *mongodb.Config, metricsProvider metrics.Provider) (*VersionedDBProvider, error) {
	logger.Debugf("constructing MongoDB VersionedDBProvider")
	mongodbInstance, err := mongodb.CreateMongoInstance(config, metricsProvider)
...
func (vdb *VersionedDB) getQueryLimit() int {
	return vdb.mongoInstance.GetConfig().QueryLimit
}
...
func (vdb *VersionedDB) ExecuteQueryWithMetadata(namespace, query string, metadata map[string]interface{}) (statedb.QueryResultsIterator, error) {
	...
	queryLimit := int32(vdb.getQueryLimit())
	limit := int32(0)
	skip := ""

...
type nsMetadataRetriever struct {
	...
	keys            []string
	executionResult []*mongodb.DocMetadata
}

// subNsMetadataRetriever implements `batch` interface and wraps the function `mongodb.BatchRetrieveDocumentMetadata`
...
func (vdb *VersionedDB) retrieveMetadata(nsKeysMap map[string][]string) (map[string][]*mongodb.DocMetadata, error) {
	...
	executionResults := make(map[string][]*mongodb.DocMetadata)
...
func retrieveNsMetadata(db *mongodb.MongoDatabase, keys []string) ([]*mongodb.DocMetadata, error) {
	...
	// accumulate results from each batch
	var executionResults []*mongodb.DocMetadata

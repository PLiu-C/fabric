...
func testGenerateLocalMSP(t *testing.T, nodeOUs bool) {
	...
	err := msp.GenerateLocalMSP(testDir, testName, nil, &ca.CA{}, &ca.CA{}, msp.PEER, nodeOUs, sigAlgo)
	...
	// generate signing CA
	signCA, err := ca.NewCA(caDir, testCAOrg, testCAName, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode, sigAlgo)
	assert.NoError(t, err, "Error generating CA")
	// generate TLS CA
	tlsCA, err := ca.NewCA(tlsCADir, testCAOrg, testCAName, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode, sigAlgo)
	assert.NoError(t, err, "Error generating CA")
	...
	err = msp.GenerateLocalMSP(testDir, testName, nil, signCA, tlsCA, msp.PEER, nodeOUs, sigAlgo)
	...
	err = msp.GenerateLocalMSP(testDir, testName, nil, signCA, tlsCA, msp.CLIENT, nodeOUs, sigAlgo)
	assert.NoError(t, err, "Failed to generate local MSP")
	// check all
	for _, file := range mspFiles {
		assert.Equal(t, true, checkForFile(file),
	...
	err = msp.GenerateLocalMSP(testDir, testName, nil, signCA, tlsCA, msp.ORDERER, nodeOUs, sigAlgo)
	assert.Error(t, err, "Should have failed with CA name 'test/fail'")
	t.Log(err)
	cleanup(testDir)
}
...
func testGenerateVerifyingMSP(t *testing.T, nodeOUs bool) {
	...
	err = msp.GenerateVerifyingMSP(mspDir, signCA, tlsCA, nodeOUs, sigAlgo)
